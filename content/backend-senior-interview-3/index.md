...
기술 부채 백로그를 *Notion 보드*에 분리 — 영향도×긴급도 매트릭스로 분기마다 상위 5개 선정. 분기 스프린트 20% 할당. 1년 후 critical 부채 0개. **교훈: 부채는 *보이지 않으면 청산 안 됨*, 가시화가 첫 단계.**
...
`docs/adr/` 디렉토리에 마크다운 ADR 30건 누적. 신규 입사자가 *왜 Kotlin인지·왜 Helm Base Chart인지* 별도 설명 없이 파악. 결정 번복 시 *기존 ADR을 Superseded* 표시로 이력 보존. **교훈: ADR은 *과거의 나*와 *미래의 동료* 모두에게 보내는 편지.**
...
이벤트 소싱 도입 후 6개월 운영 — 감사 요구는 만족했으나 *프로젝션 디버깅·재계산 비용*이 예상의 5배. 일부 도메인은 다시 CRUD로 회귀. **교훈: 실패는 *결정의 잘못*보다 *trade-off 측정 부족*이 더 흔한 원인.**
...
초기 MD5 hash 앞 7자 사용 → 1억 건 도달 시 충돌 0.3%. Snowflake → Base62로 전환 후 충돌 0건. **교훈: Hash 기반은 *생일 역설*로 결국 충돌, unique ID가 분산 환경 정석.**
...
리다이렉트 100k QPS — CDN edge에서 301 응답 캐시(1주 TTL)로 origin은 30k QPS만 도달. Redis 클러스터 hit 92%, DB 도달은 8% (8k QPS). **교훈: 리다이렉트는 *불변에 가까운 응답*, CDN 캐시 활용이 가장 큰 레버리지.**
...
피싱 URL 대량 생성 사건 — 한 IP에서 분당 5,000건 생성. Rate Limit (분당 100건) + Safe Browsing 비동기 + 위험 도메인 화이트리스트로 차단. 사후 신고 기반 자동 분석 파이프라인까지 구축. **교훈: 보안은 *다층 방어*, 단일 메커니즘만으론 우회됨.**
...
DI vs Service Locator 논쟁 — 양쪽 *의도와 장단점* 정리 후 ADR 작성. 컨벤션화하니 이후 같은 논쟁 0회. **교훈: 반복되는 논쟁은 *문서로 종결*해야 팀 시간 낭비 안 됨.**
...
주니어 PR에 코멘트 20개 → 의기소침. 다음부터 *핵심 2~3개*만 + 칭찬 1개로 변경. 본인이 *왜 그렇게 했는지* 질문 형태로. 3개월 후 본인 PR 품질 향상 + 적극성 회복. **교훈: 리뷰는 *교정*이 아니라 *대화*.**
...
핫픽스 명목으로 리뷰 없이 머지된 코드에 정합성 버그 — 즉시 수정 후 GitHub branch protection 강화 (필수 리뷰어 1명·CI 통과 필수). 핫픽스도 사후 리뷰 의무화. **교훈: *예외 프로세스*가 일상이 되면 *시스템 부재*.**
...
특정 팀원이 *트랜잭션 경계* 실수 반복 — 1:1에서 "왜 헷갈리는지" 듣고 nullable wrapper 패턴 + PR 템플릿 체크리스트 추가. 이후 같은 실수 0건. **교훈: 반복 실수는 *개인 문제*보다 *시스템·교육 부재* 신호.**
...
주 1회 *15분 라이트닝 토크* 도입. 발표자 부담 최소화 위해 슬라이드 없이 *whiteboard + 라이브 코딩*. 1년 누적 50건 공유, 신규 입사자 온보딩 자산화. **교훈: 공유는 *완벽한 발표*가 아닌 *작은 습관화*가 답.**
...
3년차에 처음 *P1 결제 장애 대응*을 단독 리드 — 패닉 속에서도 mitigation 우선, 사후 포스트모템 호스트까지. 이전엔 *코드만* 보던 시각이 *운영·비즈니스 영향*까지 확장됨. **교훈: 가장 큰 성장은 *불편한 상황에서의 책임*이 만듦.**
...
기획팀의 추가 요청 → "현재 백로그 + 신규 요청 일정 표 + Risk 도식" 제시. PM이 직접 우선순위 재조정해 일부 deprioritize. **교훈: 일정 협상은 *데이터로 보여줄 때* 합리적 결론.**
...
원격 100% 팀에서 *async-first* 원칙 — 회의 전 Notion에 안건+컨텍스트 사전 공유, 의사결정은 *댓글 + 비동기 voting*으로. 동기 회의 시간 50% 감소. **교훈: 원격은 *언제 만나느냐*가 아니라 *어떻게 비동기로 흐르느냐*.**
...
DB 샤딩 설명을 *책장과 책 비유*로 — "한 책장이 꽉 차서 같은 종류 책을 여러 책장에 나눠 두는 것". PM이 즉시 이해하고 *왜 일정이 더 걸리는지* 납득. **교훈: 기술 용어는 *벽*, 비유는 *문*.**
