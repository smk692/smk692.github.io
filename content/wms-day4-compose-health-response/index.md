...
- [feat: bootstrap api with compose and ApiResponse, GlobalExceptionHandler](https://github.com/smk692/wms/pull/5)
...
- 공통 응답 봉투 `ApiResponse` + 에러 분류 `ErrorType` + `GlobalExceptionHandler`
...
- 클라가 분기 하나(`success`)로 처리
- 에러는 자유 문자열 아니라 코드로 내려감
...
class GlobalExceptionHandler {
    ...
    fun handle(e: WmsException) =
        ResponseEntity.status(e.errorType.toHttpStatus())
            .body(ApiResponse.error(e.errorCode, e.message ?: e.errorCode))
}
```

- 진입점 늘어나도(gRPC, 메시징) 코어 모듈 무변경. HTTP 아는 건 api 어댑터뿐
- `ResponseEntityExceptionHandler` 상속 방법 있지만 의도적으로 안 씀
  - WebFlux 는 프레임워크 예외(405/415/바인딩)가 대부분 `ResponseStatusException` 하위 타입 → 폴백 핸들러 하나로 커버. 매핑 없는 경로 404 도 우리 봉투로 나오는 거 실측
  - 상속하면 `ProblemDetail`(RFC 9457)이 들어와 우리 단일 봉투랑 이중 규격 됨
...
> "에러 분류는 프레임워크 무관한 코어에, HTTP 상태 매핑은 어댑터 한 지점에 격리함. WebFlux 예외가 ResponseStatusException 계층으로 통일된 점을 근거로 ResponseEntityExceptionHandler 상속 없이 단일 응답 봉투를 유지함"
